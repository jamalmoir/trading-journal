...
export const fetchJournalsSuccess = (journals: Types.Journal[]) =>
  action(FETCH_JOURNALS_SUCCESS, journals)
export const fetchJournalsFailure = (message: string) =>
  action(FETCH_JOURNALS_FAILURE, message)

export const createJournalRequest = () => action(CREATE_JOURNAL_REQUEST)
export const createJournalSuccess = (journal: Types.Journal) =>
  action(CREATE_JOURNAL_SUCCESS, journal)
export const createJournalFailure = (message: string) =>
  action(CREATE_JOURNAL_FAILURE, message)

export const modifyJournalRequest = () => action(MODIFY_JOURNAL_REQUEST)
export const modifyJournalSuccess = (journal: Types.Journal) =>
  action(MODIFY_JOURNAL_SUCCESS, journal)
export const modifyJournalFailure = (message: string) =>
  action(MODIFY_JOURNAL_FAILURE, message)

export const deleteJournalRequest = () => action(DELETE_JOURNAL_REQUEST)
export const deleteJournalSuccess = (journal: Types.Journal) =>
  action(DELETE_JOURNAL_SUCCESS, journal)
export const deleteJournalFailure = (message: string) =>
  action(DELETE_JOURNAL_FAILURE, message)

export const setActiveJournal = (journal: Types.Journal) =>
  action(SET_ACTIVE_JOURNAL, journal)
  ...
  return (
    ...
    firestore
      ...
      .then((snapshot: firestore.QuerySnapshot) =>
        dispatch(fetchJournalsSuccess(extractJournals(snapshot)))
      )
      .catch((err: Error) =>
        dispatch(fetchJournalsFailure(err.message || 'Something went wrong.'))
      )
  }
}
...
export const deleteJournal = (journal: Types.Journal) => {
  return (
    ...
    firestore
      ...
      .catch((err: Error) =>
        dispatch(deleteJournalFailure(err.message || 'Something went wrong.'))
      )
  }
}
