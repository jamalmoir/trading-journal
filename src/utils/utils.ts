...
export const stringsToTags = (strings: string[]): Tag[] =>
  Array.isArray(strings) ? strings.map(s => ({ id: s, name: s })) : []
...
export const controlsValid = (controls: Types.InputControls) =>
  Object.keys(controls)
    .map(key => controls[key].errors.length)
    .every(v => !v)
